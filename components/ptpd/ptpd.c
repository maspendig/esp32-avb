...
static bool is_selected_source_valid(FAR struct ptp_state_s *state)
{
  ...
  if ((state->selected_source.header.messagetype & PTP_MASK)
      != PTP_ANNOUNCE)
    {
...
static int ptp_send_announce(FAR struct ptp_state_s *state)
{
  ...
  msg.header.messagetype = PTP_ANNOUNCE;
...
static int ptp_send_sync(FAR struct ptp_state_s *state)
{
  ...
  msg.header.messagetype = PTP_SYNC;
  msg.header.messagelength[1] = sizeof(msg);
...
#endif // !ESP_PTP
  timespec_to_ptp_format(&ts, msg.origintimestamp);
  msg.header.messagetype = PTP_FOLLOW_UP;
...
static int ptp_send_delay_req(FAR struct ptp_state_s *state)
{
  ...
  req.header.messagetype = PTP_DELAY_REQ;
                                 ...
                                 ssize_t length)
{
  ...
  switch (state->rxbuf.header.messagetype & PTP_MASK)
  {
    ...
    /* messages proceed by server and client */
  ...
  case PTP_PEER_DELAY_RESP:
    ptpinfo("Got peer delay resp, seq %ld\n",
            (long)ptp_get_sequence(&state->rxbuf.header));
    return OK;

  case PTP_PEER_DELAY_FOLLOW_UP:
    ptpinfo("Got peer delay follow up, seq %ld\n",
            (long)ptp_get_sequence(&state->rxbuf.header));
    return OK;
  default:
      ptpinfo("Ignoring unknown PTP packet type: 0x%02x",
              state->rxbuf.header.messagetype & PTP_MASK);
      return OK;
  }
}
